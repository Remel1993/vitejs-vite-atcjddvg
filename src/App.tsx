...
export default function App() {
  ...
  useEffect(() => {
    setIsClient(true);
    try {
      const savedArchive = window.localStorage.getItem('hub-elite-archive');
      if (savedArchive) setArchive(JSON.parse(savedArchive));

      const savedComps = window.localStorage.getItem('hub-elite-comps');
      if (savedComps) setComps(JSON.parse(savedComps));
    } catch (e) {
