...
const COUNTRY_CODES = {
  ...
  'canadá': 'ca', 'cape verde': 'cv', 
  'cabo verde': 'cv', 'central african republic': 'cf',
  ...
  'finlandia': 'fi', 
  'france': 'fr', 'francia': 'fr', 'gabon': 'ga', 'gambia': 'gm',
  ...
  'nueva zelanda': 'nz', 'nicaragua': 'ni', 'niger': 'ne', 'nigeria': 'ng', 'north korea': 
  'kp',
  ...
  'slovenia': 'si', 'eslovenia': 'si', 'somalia': 'so', 'south 
  africa': 'za',
  ...
  'usa': 'us', 'estados 
  unidos': 'us', 'uruguay': 'uy', 'uzbekistan': 'uz', 'venezuela': 've',
...
const Shield = ({ color1, color2, initial, size = 'md', isFlag = false }) => {
  const dims = size === 'lg' ?
  'w-20 h-24' : size === 'sm' ? 'w-8 h-10' : size === 'xs' ? 'w-5 h-6' : 'w-12 h-14';
  const imgDims = size === 'lg' ? 'w-20 h-14' : size === 'sm' ?
  'w-8 h-6' : size === 'xs' ? 'w-5 h-4' : 'w-12 h-8';
  const fontSize = size === 'lg' ?
  'text-2xl' : size === 'sm' ? 'text-[10px]' : size === 'xs' ? 'text-[8px]' : 'text-sm';
  ...
  font-black text-white mix-blend-difference italic drop-shadow-md`}>{safeInitial}</span>
        </div>
      </div>
    );
  }
...
const Confetti = () => (
  ...
  infinite`, animationDelay: `${Math.random() * 2}s`
        }}
      />
    ))}
...
const AttrStepper = ({ label, val, min, max, onUpdate }) => (
  <div className='flex flex-col items-center bg-black/40 rounded-xl p-1.5 border border-white/10'>
    ...
    <div className='flex items-center gap-2 w-full justify-center'>
      ...
      <button onClick={() => onUpdate(Math.min(max, val + 1))} className='w-5 h-5 bg-slate-800/80 hover:bg-slate-700 rounded text-white text-xs font-bold active:scale-95 flex items-center justify-center 
...
const PenaltyDots = ({ history }) => {
  ...
  const startIdx = totalLen % 5 === 0 && totalLen > 0 ?
  totalLen - 5 : totalLen - (totalLen % 5);
  const visibleHistory = history && totalLen > 0 ?
    ...
    <div className="flex justify-center gap-[3px] mb-2 min-h-[14px]">
      {visibleHistory.map((h, i) => {
        ...
        return (
          <div
            key={globalIdx}
            style={isNewest ? { animation: 'penDotPop 0.35s cubic-bezier(0.34,1.56,0.64,1) 
  both' } : {}}
...
(
  ...
  <h3 className='text-sm font-black uppercase italic truncate text-white'>{entry.name}</h3>
                  <p className='text-[10px] text-slate-300 font-bold'>{entry.date} • Campeón: {entry.winner?.name ||
'Desconocido'}</p>
                </div>
  ...
  backdrop-blur-md rounded-[2.5rem] border border-yellow-500/40 p-6 relative overflow-hidden shadow-2xl'>
            ...
            {selectedArchiveEntry.type === 'league' && selectedArchiveEntry.teams && (
              <div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl p-4 border border-white/5 text-left">
               ...
               <div className="space-y-2">
                  {[...selectedArchiveEntry.teams].sort((a,b)=>b.pts-a.pts ||
(b.gf-b.ga)-(a.gf-a.ga)).slice(0, 4).map((t, i) => (
  ...
  {selectedArchiveEntry.type !== 'league' && selectedArchiveEntry.bracket?.Final && selectedArchiveEntry.bracket.Final[0] && (
               <div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl p-4 border border-white/5">
                 ...
                 {(() => {
                   const 
  finalMatch = selectedArchiveEntry.bracket.Final[0];
...
return (
                 ...
                 <span className="text-[9px] font-bold uppercase truncate">{home?.name}</span>
                        ...
                        <div className="flex flex-col items-center">
                           <span 
className="bg-slate-900/50 px-3 py-1 rounded text-[11px] font-black tabular-nums">{finalMatch.sh} - {finalMatch.sa}</span>
...
const RulesView = ({ setView }) => (
  <div className='flex-grow px-4 pb-8 flex flex-col'>
    ...
    <p className='text-[11px] font-bold text-slate-200 leading-relaxed'>Cada partido se decide por turnos de ataque. Los equipos tienen un número limitado de tiros (OPORT) basado en su fortaleza.</p>
      ...
      <div className='bg-slate-900/30 backdrop-blur-md p-6 rounded-3xl border border-white/10 shadow-lg'>
        <h4 className='text-xs font-black uppercase italic text-blue-400 mb-2'>2.
Ataque y Defensa</h4>
        <p className='text-[11px] font-bold text-slate-200 leading-relaxed'>Para marcar gol, el atacante debe sacar un número menor o igual a su ATK.
Si lo logra, el portero rival debe sacar un número <strong className='text-white'>menor o igual a su DEF</strong> para detenerlo.
Si saca mayor, es GOL.</p>
      </div>
      <div className='bg-slate-900/30 backdrop-blur-md p-6 rounded-3xl border border-white/10 shadow-lg'>
        <h4 className='text-xs font-black uppercase italic text-yellow-400 mb-2'>3.
Formatos y Fechas</h4>
        <p className='text-[11px] font-bold text-slate-200 leading-relaxed'>Ligas de 18 o 20 equipos calculan sus jornadas automáticamente.
Las copas tienen grupos, ida, vuelta, y una Final a partido único.</p>
      </div>
      <div className='bg-slate-900/30 backdrop-blur-md p-6 rounded-3xl border border-white/10 shadow-lg'>
        <h4 className='text-xs font-black uppercase italic text-purple-400 mb-2'>4.
Tanda de Penales</h4>
        <p className='text-[11px] font-bold text-slate-200 leading-relaxed'>Si hay empate global, se van a penales.
Verás el registro individual de cada tiro (✓ o ✗).
Si persiste el empate en los 5 tiros, ¡Muerte Súbita!</p>
      </div>
    </div>
  </div>
);
  ...
  <div className='flex-grow flex flex-col px-4 pb-8'>
    ...
    <div className='space-y-8 flex-grow'>
     ...
     <div className='w-14 h-14 rounded-2xl bg-yellow-500/20 flex items-center justify-center text-yellow-400 shadow-[inset_0_0_15px_rgba(234,179,8,0.2)] group-hover:scale-110 transition-transform duration-300'><History size={28} /></div>
        ...
        <button onClick={() => setView('rules')} className='p-5 
bg-slate-900/30 backdrop-blur-md rounded-[2rem] border border-white/10 flex flex-col items-center gap-3 hover:bg-slate-800/40 hover:border-blue-500/30 hover:shadow-[0_0_25px_rgba(59,130,246,0.25)] active:scale-[0.98] transition-all duration-300 group'>
      ...
      <section>
        ...
        <div className='grid grid-cols-2 gap-4'>
          {['C1', 'C2'].map(id => (
            <button key={id} onClick={() => { setActiveCompId(id);
setView('competition'); setCompView('main'); }} className='p-6 bg-slate-900/30 backdrop-blur-md rounded-[2rem] border border-white/10 flex flex-col items-center gap-4 hover:bg-slate-800/40 hover:border-white/20 hover:shadow-[0_0_25px_rgba(255,255,255,0.1)] active:scale-[0.98] transition-all duration-300 group'>
              <div className={`w-14 h-14 rounded-full bg-slate-950/40 flex items-center justify-center text-white border border-white/5 shadow-inner group-hover:scale-110 transition-transform duration-300 ${id === 'C1' ?
'drop-shadow-[0_0_15px_rgba(96,165,250,0.5)]' : 'drop-shadow-[0_0_15px_rgba(52,211,153,0.5)]'}`}>
                {id === 'C1' ?
<Trophy size={28} className="text-blue-400" /> : <Globe size={28} className="text-emerald-400" />}
              </div>
              <div className='text-center'>
                <h4 className='text-[11px] font-black uppercase italic text-white tracking-wide'>{comps[id]?.name}</h4>
                <span className='text-[8px] font-bold px-2 py-0.5 rounded-md bg-white/20 backdrop-blur-sm text-slate-100 mt-2 inline-block uppercase tracking-wider'>{comps[id]?.phase === 'groups' ?
'Fase Grupos' : comps[id]?.phase}</span>
              </div>
            </button>
          ))}
        </div>
...
gap-4'>
  ...
  <p className='text-[10px] text-slate-300 font-bold uppercase tracking-widest mt-1'>Selecciona tu liga local</p>
          ...
          <motion.div animate={{ rotate: showLeagues ?
90 : 0 }} transition={{ type: "spring", stiffness: 200 }} className='w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-slate-200 group-hover:bg-emerald-500/20 group-hover:text-emerald-400 transition-colors'>
            <ArrowRight size={20} />
          </motion.div>
        </button>
...
height: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className='overflow-hidden'>
              ...
              return (
                    <motion.button initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} key={id} onClick={() => { setActiveCompId(id); setView('competition');
setCompView('main'); }} className='p-4 bg-slate-900/30 backdrop-blur-md rounded-[1.5rem] border border-white/5 flex items-center justify-between hover:bg-slate-800/50 hover:border-emerald-500/20 hover:shadow-[0_0_15px_rgba(52,211,153,0.1)] active:scale-[0.98] transition-all duration-200 group'>
     ...
     <div className='text-left'>
                          ...
                          <p className='text-[9px] font-bold text-slate-400 uppercase tracking-wider mt-0.5'>{isConf ?
`Jornada ${(comp.matchday || 0) + 1} de ${(comp.teams.length - 1) * 2}` : 'No Inicializada'}</p>
                        </div>
                      </div>
    ...
    <footer 
className='py-6 text-center opacity-40'><p className='text-[9px] font-black uppercase tracking-widest text-white drop-shadow-md'>Powered by Dice Engine v5.0</p></footer>
  </div>
)};
...
return (
         ...
         <button onClick={() => { setDraft(prev => ({...prev, ...drawModal.drawData})); setDrawModal(null);
}} className='flex-[2] bg-blue-600 py-4 rounded-xl font-black uppercase italic text-white active:scale-95 shadow-lg shadow-blue-500/20 transition-all'>Confirmar y Guardar</button>
                       </div>
                  </div>
              )}
          </div>
...
'Miscelánea', color: 'pink'}
   ...
   <button onClick={() => handleDrawUI('auto')} disabled={hasStarted} className={`p-3 border rounded-2xl text-[8px] font-black uppercase italic flex flex-col items-center justify-center gap-1 transition-all backdrop-blur-md ${hasStarted ?
'opacity-40 cursor-not-allowed bg-yellow-900/20 border-yellow-500/10 text-yellow-500/50' : 'bg-yellow-600/20 text-yellow-200 border-yellow-500/40 hover:bg-yellow-600/40 active:scale-95'}`}>
              <Wand2 size={16}/> Auto-Rellenar
            </button>
            <button onClick={() => handleDrawUI('shuffle')} disabled={hasStarted} className={`p-3 border rounded-2xl text-[8px] font-black uppercase italic flex flex-col items-center justify-center gap-1 transition-all backdrop-blur-md ${hasStarted ?
'opacity-40 cursor-not-allowed bg-emerald-900/20 border-emerald-500/10 text-emerald-500/50' : 'bg-emerald-600/20 text-emerald-200 border-emerald-500/40 hover:bg-emerald-600/40 active:scale-95'}`}>
...
export default function App() {
  ...
  // MANEJO DE ARCHIVO PERSISTENTE (LOCALSTORAGE)
  const [archive, setArchive] = useState(() => {
    try {
      const saved = window.localStorage.getItem('hub-elite-archive');
  ...
  useEffect(() => {
    window.localStorage.setItem('hub-elite-archive', JSON.stringify(archive));
  }, [archive]);
  ...
  // MANEJO DE COMPETICIONES PERSISTENTES (LOCALSTORAGE)
  const [comps, setComps] = useState(() => {
    try {
      const saved = window.localStorage.getItem('hub-elite-comps');
      if (saved) {
        return JSON.parse(saved);
      }
    } catch (e) {
      console.warn('No se pudo cargar las competiciones', e);
    }
  ...
  useEffect(() => {
    window.localStorage.setItem('hub-elite-comps', JSON.stringify(comps));
  }, [comps]);
...
setTimeout(() => {
    ...
    if (penPhase === 'att') {
            if (die <= attacker.att) {
              ...
              newLogs.unshift('❌ ' + 
attacker.name + ' sacó un ' + die + '. El penalti se va fuera.');
...
let { scoreH, scoreA, phase: newPhase } = prev;
        ...
        if (newPhase === 'att') {
          if (die <= attacker.att) { newLogs.unshift('🎯 ' + attacker.name + ' sacó un ' + die + '. ¡Va a portería!');
newPhase = 'gk'; } 
          else { newLogs.unshift('❌ ' + attacker.name + ' sacó un ' + die + '. El disparo se va fuera.');
return advanceTurn({ ...prev, lastDie: die, logs: newLogs, phase: 'att' });
}
        } else {
          if (die > defender.def) { newLogs.unshift('⚽ ¡GOOOOOOL de ' + attacker.name + '! El dado marcó ' + die + '.');
isHome ? scoreH++ : scoreA++; } 
  ...
  const advanceTurn = (state) => {
    let nextOppH = state.turn === 'H' ?
state.oppH - 1 : state.oppH;
...
const currentRound = Array.isArray(schedule) ? schedule[activeComp.matchday] : [];
      const results = Array.isArray(currentRound) ?
currentRound.map(m => {
        ...
        let sh = 0, sa = 0;
        for(let i=0; i<(h?.opp || 0); i++) if(Math.floor(Math.random()*6)+1 <= (h?.att || 0) && Math.floor(Math.random()*6)+1 > (a?.def || 
0)) sh++;
...
const updatedTeams = Array.isArray(activeComp.teams) ? activeComp.teams.map(t => {
        ...
        const w = gf > ga ? 1 : 0; const d 
= gf === ga ? 1 : 0; const l = gf < ga ? 1 : 0;
...
const isFinished = activeComp.matchday === schedule.length - 1;
      const updatedComp = { teams: updatedTeams, history: [{ day: activeComp.matchday + 1, results }, ...activeComp.history], matchday: isFinished ?
activeComp.matchday : activeComp.matchday + 1, showWinner: isFinished };
...
const updatedTeams = activeComp.teams.map(t => {
          ...
          const w = gf > ga ? 1 : 0; const 
d = gf === ga ? 1 : 0; const l = gf < ga ? 1 : 0;
          return { ...t, p: t.p + 1, w: t.w + w, d: t.d + d, l: t.l + l, gf: t.gf + gf, ga: t.ga + ga, pts: t.pts + (w * 3 + d) };
        });
...
matchesToProcess.forEach(m => {
          ...
          if (m.hId === matchState.home.id && m.aId === matchState.away.id) {
            ...
            sh = matchState.scoreA; sa = matchState.scoreH; penH = matchState.penalties?.scoreA; 
penA = matchState.penalties?.scoreH; 
          } else {
            const h = activeComp.teams.find(t => t.id === (isVuelta ? m.aId : m.hId));
            const a = activeComp.teams.find(t => t.id === (isVuelta ? m.hId : m.aId));
            let simH = 0, simA = 0;
            for(let i=0; i<h.opp; i++) if(Math.floor(Math.random()*6)+1 <= h.att && Math.floor(Math.random()*6)+1 
> a.def) simH++;
...
if (simPen(a_orig.att, h_orig.def)) simPenA++;
              ...
              while (simPenH === simPenA) { if (simPen(h_orig.att, a_orig.def)) simPenH++;
if (simPen(a_orig.att, h_orig.def)) simPenA++; }
              penH = simPenH;
penA = simPenA;
            }
          }
          if (isVuelta) { m.sh2 = sh;
m.sa2 = sa; } else { m.sh = sh; m.sa = sa;
}
          if (penH !== undefined) { m.penH = penH;
m.penA = penA; }
...
if (!winners.includes(activeComp.userTeamId)) isUserDisqualified = true;
          ...
          if (phase === 'Octavos') { nextPhase = 'Cuartos';
newBracket.Cuartos = Array(4).fill(0).map((_, i) => ({ id: 'C'+(i+1), hId: winners[i*2], aId: winners[i*2+1], sh: null, sa: null, penH: null, penA: null, sh2: null, sa2: null }));
          ...
          else if (phase === 'Cuartos') { nextPhase = 'Semis';
newBracket.Semis = Array(2).fill(0).map((_, i) => ({ id: 'S'+(i+1), hId: winners[i*2], aId: winners[i*2+1], sh: null, sa: null, penH: null, penA: null, sh2: null, sa2: null }));
          ...
          else if (phase === 'Semis') { nextPhase = 'Final';
...
const resetCompetition = (compId) => {
    setComps(prev => {
      ...
      if (comp.type === 'league') {
        const pres = compId === 'L1' ? 'ES' : compId === 'L2' ? 'IT' : compId === 'L3' ? 'EN' : compId === 'L4' ? 'DE' : 
compId === 'L5' ? 'NL' : compId === 'L6' ? 'FR' : 'MI';
...
if (!activeComp.teams || activeComp.teams.length === 0) {
   ...
   <div className='space-y-4 w-full max-w-xs'>
            ...
            <button onClick={() => updateActiveComp(getShuffleData(activeCompId))} className='w-full bg-emerald-600/80 backdrop-blur-md hover:bg-emerald-500 text-white py-4 rounded-2xl text-[11px] font-black uppercase italic tracking-widest shadow-xl transition-all active:scale-95 flex 
...
if (m) return m;
        ...
        const matchArray = Array.isArray(activeComp.bracket[activeComp.phase]) ?
activeComp.bracket[activeComp.phase] : [activeComp.bracket[activeComp.phase]];
...
uppercase mb-2 tracking-tighter'>¡CAMPEÓN!</h2>
...
font-black uppercase italic">Llaves</span>
                    </button>
                  )}
                </div>
     ...
     <div className='w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center text-red-500 mx-auto mb-6'><ShieldIcon size={40} /></div>
                ...
                <p className='text-[10px] text-slate-400 font-bold uppercase text-center mb-8 tracking-widest'>Tu equipo ha sido eliminado de la competición.
Selecciona un nuevo equipo para seguir jugando.</p>
        ...
        <header className='flex items-center gap-3 mb-6'>
          <button onClick={() => setView('hub')} className='p-2 bg-slate-900/30 backdrop-blur-md 
rounded-xl text-slate-200 border border-white/10 active:scale-95 transition-all'><ChevronLeft /></button>
          <div className='flex-grow'>
       ...
       <MenuButton icon={<BarChart3 size={18} className='text-emerald-400'/>} label="Stats" onClick={() => setCompView('stats')} />
           ...
           <MenuButton icon={<History size={18} className='text-yellow-400'/>} label="Result."
onClick={() => setCompView('results')} />
           {activeComp.type !== 'league' ?
(
        ...
        {activeComp.type !== 'league' && activeComp.phase === 'groups' && Array.isArray(activeComp.groups) && (
          <div className='grid grid-cols-1 gap-6 mb-8'>
           ...
           <section key={gi} className='bg-slate-900/30 backdrop-blur-md rounded-[2rem] p-4 border border-white/10 shadow-lg'>
                ...
                <div className='space-y-1.5'>
                  {activeComp.teams.filter(t => group.teamIds.includes(t.id)).sort((a, b) => b.pts - a.pts ||
(b.gf - b.ga) - (a.gf - a.ga)).map((t, i) => (
                    <div key={t.id} className={'flex items-center gap-3 p-2 rounded-xl ' + (t.id === activeComp.userTeamId ? 'bg-blue-600/40 border border-blue-400/50 shadow-inner' : 'bg-black/30')}>
                      ...
                      <Shield color1={t.color1} color2={t.color2} initial={t.name} 
size='sm' isFlag={t.isFlag} />
    ...
    <h3 className='text-[10px] font-black uppercase text-slate-200 mb-3 flex items-center gap-2 drop-shadow-md'><BarChart3 size={12} /> Top Clasificación</h3>
            <div className='space-y-1.5'>
              {sortedTeams.slice(0, 6).map((t, i) => (
                <div key={t.id} className={'flex items-center gap-3 p-2 rounded-xl ' + (t.id === activeComp.userTeamId ?
'bg-blue-600/40 border border-blue-400/50 shadow-inner' : 'bg-black/30')}>
                  <span className='text-[10px] font-black text-slate-300 italic w-4'>{i+1}</span>
                  <Shield color1={t?.color1} color2={t?.color2} initial={t?.name} size='sm' isFlag={t?.isFlag} />
                  <span className='text-[11px] font-bold uppercase truncate flex-grow italic text-white drop-shadow-sm'>{t?.name}</span>
                  <span className='text-[10px] font-black bg-slate-800/60 
px-2 py-0.5 rounded-md text-emerald-400 border border-white/10'>{t.pts} PTS</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className='bg-gradient-to-br from-blue-700/80 to-indigo-900/80 backdrop-blur-md rounded-[2.5rem] p-6 shadow-2xl relative overflow-hidden border border-white/20'>
           ...
           <div className='flex justify-between items-center mb-6'>
              <div className='text-center w-24'>
                ...
                {homeTeam?.id === userTeam?.id && <span className='text-[7px] font-black 
bg-white/30 px-1.5 py-0.5 rounded uppercase backdrop-blur-sm'>Tu Equipo</span>}
              </div>
              <div className='flex flex-col items-center'>
            ...
            <button onClick={() => startMatch(homeId, awayId)} className='w-full bg-white/95 text-blue-900 py-4 rounded-2xl text-xs font-black uppercase italic tracking-widest shadow-xl active:scale-95 transition-all flex flex-col items-center justify-center'>
              <span>{activeComp.phase === 'Final' ?
'Gran Final' : ('Jugar ' + (activeComp.type === 'league' || activeComp.phase === 'groups' ? 'Jornada ' + (activeComp.matchday + 1) : activeComp.phase + (activeCompId === 'C1' ? (activeComp.matchday % 2 === 0 ? ' (Ida)' : ' (Vuelta)') : '')))}</span>
    ...
    if (compView === 'stats') return (
      <div className='flex-grow px-4 pb-20'>
        ...
        {activeComp.type === 'league' ? (
          <div className='bg-slate-900/30 backdrop-blur-md rounded-[2rem] border 
border-white/10 overflow-x-auto custom-scrollbar relative shadow-xl'>
      ...
      <th className='p-3 sticky z-50 bg-[#0f172a]' style={{ left: '40px', minWidth: '130px' }}>Equipo</th>
              ...
              <tbody className='divide-y divide-white/5'>
                {Array.isArray(sortedTeams) && sortedTeams.map((t, i) => (
                  <tr key={t.id} className={t.id === activeComp.userTeamId ?
'bg-blue-600/30' : ''}>
                    ...
                    <td 
className='p-3 text-center text-[10px] font-bold sticky z-40 bg-[#0f172a] border-r border-white/10' style={{ left: '170px' }}>{t.p}</td>
...
sticky top-0 z-50'>
             ...
             <th className='p-3 sticky z-50 bg-[#0f172a] text-center border-r border-white/10' style={{ left: '170px', minWidth: '40px' }}>PJ</th>
                  ...
                  <tbody className='divide-y divide-white/5'>
                    {Array.isArray(activeComp.teams) && activeComp.teams.filter(t => Array.isArray(group.teamIds) && group.teamIds.includes(t.id)).sort((a, b) => b.pts - a.pts ||
(b.gf - b.ga) - (a.gf - a.ga)).map((t, i) => (
                      <tr key={t.id} className={t.id === activeComp.userTeamId ? 'bg-blue-600/30' : ''}>
                        <td className='p-3 text-[10px] font-black italic text-slate-300 sticky z-40 bg-[#0f172a]' style={{ left: 0 }}>{i+1}</td>
                        <td 
className='p-3 flex items-center gap-2 sticky z-40 bg-[#0f172a]' style={{ left: '40px', minWidth: '130px' }}><Shield color1={t?.color1} color2={t?.color2} initial={t?.name} size='xs' isFlag={t?.isFlag} /><span className='text-[10px] font-bold uppercase truncate italic max-w-[80px]'>{t?.name}</span></td>
                        <td className='p-3 text-center text-[10px] font-bold sticky z-40 bg-[#0f172a] border-r border-white/10' style={{ left: '170px' }}>{t.p}</td>
                        <td className='p-3 text-center text-[10px] font-bold'>{t.w}</td><td className='p-3 text-center text-[10px] font-bold'>{t.d}</td><td className='p-3 text-center text-[10px] font-bold'>{t.l}</td><td className='p-3 text-center 
text-[10px] font-bold'>{t.gf}</td><td className='p-3 text-center text-[10px] font-bold'>{t.ga}</td><td className='p-3 text-center text-[10px] font-bold'>{t.gf - t.ga}</td><td className='p-3 text-center text-[10px] font-black text-emerald-400'>{t.pts}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
if (compView === 'results') return (
      <div className='flex-grow px-4 pb-20'>
        ...
        <div className='space-y-4'>
          {(!Array.isArray(activeComp.history) || activeComp.history.length === 0) && <div className='text-center py-20 text-slate-300 
bg-slate-900/30 backdrop-blur-md rounded-[2rem] border border-white/5 italic font-bold uppercase text-[10px] shadow-lg'>No hay partidos jugados aún.</div>}
...
&& h.results.map((r, ri) => {
                  const home = Array.isArray(activeComp.teams) ?
activeComp.teams.find(t => t.id === r.hId) : null;
...
return (
                    <div key={ri} className='flex items-center justify-between bg-black/30 p-3 rounded-2xl border border-white/5'>
                      ...
                      <div className='bg-slate-800/60 backdrop-blur-sm px-3 py-1 rounded-lg text-xs font-black italic tabular-nums flex 
flex-col items-center border border-white/10'>
                        <span>{r.sh} - {r.sa}</span>
...
if (compView === 'calendar') return (
      ...
      (() => {
              ...
              <div key={ri} className={'bg-slate-900/30 backdrop-blur-md rounded-3xl p-4 border border-white/10 shadow-lg 
' + (ri === activeComp.matchday ? 'ring-2 ring-blue-500/50' : 'opacity-80')}>
                <div className='flex justify-between items-center mb-3'>
                  <h3 className='text-[9px] font-black uppercase text-slate-300'>Jornada {ri + 1} {ri === activeComp.matchday && '(Actual)'}</h3>
                  <span className={'text-[7px] font-black uppercase px-2 py-0.5 rounded-full ' + (ri < activeComp.matchday ? 'bg-emerald-500/30 text-emerald-300' : ri === activeComp.matchday ? 'bg-blue-500/40 text-blue-200' : 
'bg-slate-800/80 text-slate-300')}>{ri < activeComp.matchday ? 'Finalizado' : ri === activeComp.matchday ?
'En Curso' : 'Próximo'}</span>
                </div>
                <div className='space-y-2'>
                  {round.map((m, mi) => {
            ...
            const result = activeComp.history.find(h => h.day === 'Jornada ' + (ri + 1))?.results.find(r => (r.hId === m.homeId && r.aId === m.awayId) || (r.hId === m.awayId && r.aId === m.homeId));
                ...
                <div className='flex items-center gap-2 w-24'><Shield color1={home?.color1} color2={home?.color2} initial={home?.name} size='xs' isFlag={home?.isFlag} /><span className='text-[9px] font-bold uppercase truncate italic'>{home?.name}</span></div>
                        ...
                        <div 
className='flex items-center gap-2 w-24 justify-end'><span className='text-[9px] font-bold uppercase truncate italic'>{away?.name}</span><Shield color1={away?.color1} color2={away?.color2} initial={away?.name} size='xs' isFlag={away?.isFlag} /></div>
                      </div>
                    );
})}
                </div>
...
const isWorldCup = activeCompId === 'C2';
                    ...
                    return (
                      <div 
key={gi} className='bg-slate-900/30 backdrop-blur-md rounded-3xl p-4 border border-white/10 shadow-lg'>
...
return [...rounds.filter(r => r.ri === curIdx), ...rounds.filter(r => r.ri > curIdx), ...rounds.filter(r => r.ri < curIdx).reverse()];
               ...
               return (
                              ...
                              <span className='text-[8px] font-black uppercase text-slate-300 italic'>Jornada {ri + 1}</span>
                                  <span className={'text-[7px] font-black uppercase px-2 py-0.5 rounded-full ' + (isPast ? 'bg-emerald-500/30 text-emerald-300' : isCur ? 'bg-blue-500/40 text-blue-200' : 'bg-slate-800/80 text-slate-300')}>{isPast ? 'Finalizado' : isCur ?
'En Curso' : 'Próximo'}</span>
                                </div>
                                {round.map((m, mi) => {
    ...
    const home = activeComp.teams.find(t => t.id === m.homeId); const away = activeComp.teams.find(t => t.id === m.awayId);
               ...
               return (
                                    <div key={mi} className='flex items-center justify-between py-1 border-b border-white/10 last:border-0'>
                                      <div className='flex 
items-center gap-2 w-20'><Shield color1={home?.color1} color2={home?.color2} initial={home?.name} size='xs' isFlag={home?.isFlag} /><span className='text-[9px] font-bold uppercase italic truncate'>{home?.name}</span></div>
                                      <div className='flex flex-col items-center'>
                                        {result ?
(
                                          <div className='flex flex-col items-center'>
...
const phaseIdx = phases.indexOf(phase);
                    let status = phaseIdx < currentPhaseIdx ? 'Finalizado' : phaseIdx === currentPhaseIdx ?
'En Curso' : 'Próximo';
...
<h3 className='text-[9px] font-black uppercase text-slate-300'>{phase}</h3>
   ...
   <div className='space-y-2'>
                          {matchArray.map((m, mi) => {
                            if (!m) return null;
                            const home = activeComp.teams.find(t => t.id === m.hId); 
const away = activeComp.teams.find(t => t.id === m.aId);
                            const isChampions = activeCompId === 'C1';
                            const isPlayedIda = m.sh !== null;
const isPlayedVuelta = m.sh2 !== null;
      ...
      {isPlayedIda ? <div className='flex items-center gap-2 bg-slate-800/60 px-1.5 rounded'><span className='text-[10px] font-black tabular-nums'>{m.sh} - {m.sa}</span><span className='text-[7px] font-bold text-slate-400 uppercase italic'>Ida</span></div> : <span className='text-[8px] font-black text-slate-500 italic'>VS (Ida)</span>}
                                  ...
                                  <div 
className='flex items-center gap-2 w-28 justify-end'><span className='text-[9px] font-bold uppercase truncate italic text-right'>{away?.name ||
'TBD'}</span><Shield color1={away?.color1} color2={away?.color2} initial={away?.name} size='xs' isFlag={away?.isFlag} /></div>
                                </div>
     ...
     <div className='flex flex-col items-center flex-1'>
                                  ...
                                  <div className='flex items-center gap-2 bg-slate-800/60 px-1.5 rounded'><span className='text-[10px] font-black tabular-nums'>{m.sh2} - {m.sa2}</span><span className='text-[7px] font-bold text-slate-400 uppercase italic'>Vuelta</span></div>
                                        ...
                                        <span className='text-[8px] font-black text-blue-300 uppercase italic bg-blue-900/40 px-1.5 rounded'>Global: {m.sh + m.sh2} - {m.sa + m.sa2}</span>
                                            {m.penH 
!== undefined && m.penH !== null && <span className='text-[7px] text-blue-200 font-bold'>(pen {m.penH}-{m.penA})</span>}
                                          </div>
                                        </div>
            ...
            <div className='flex items-center gap-2 w-28 justify-end'><span className='text-[9px] font-bold uppercase truncate italic text-right'>{home?.name ||
'TBD'}</span><Shield color1={home?.color1} color2={home?.color2} initial={home?.name} size='xs' isFlag={home?.isFlag} /></div>
                                  </div>
                                )}
...
if (compView === 'bracket') return (
      <div className='flex-grow px-4 pb-20'>
        ...
        {!activeComp.bracket ? (
          <div className='text-center py-20 text-slate-300 font-black bg-slate-900/30 backdrop-blur-md rounded-[2rem] border 
border-white/10 uppercase italic text-[10px] shadow-lg'>Las eliminatorias se generarán al finalizar la fase de grupos.</div>
        ) : (
          <div className='flex gap-8 overflow-x-auto custom-scrollbar pb-8'>
            {['Octavos', 'Cuartos', 'Semis', 'Final'].filter(p => activeComp.bracket[p]).map(phase => (
              <div key={phase} className='min-w-[280px] flex-shrink-0'>
                <h3 className='text-[10px] font-black uppercase text-blue-300 mb-4 px-3 border-l-2 border-blue-500 bg-slate-900/40 
backdrop-blur-md rounded-r-xl py-1 drop-shadow-md'>{phase}</h3>
                <div className='grid grid-cols-1 gap-3'>
                  {(Array.isArray(activeComp.bracket[phase]) ?
activeComp.bracket[phase] : [activeComp.bracket[phase]]).filter(m => m !== null).map((m, mi) => {
 ...
 if (m.sh > m.sa) winner = h; else if (m.sa > m.sh) winner = a; else if (m.penH !== null && m.penH !== undefined) winner = m.penH > m.penA ? h : a;
                  ...
                  <div key={mi} className='bg-slate-900/30 backdrop-blur-md rounded-2xl p-3 border border-white/10 flex flex-col gap-2 shadow-lg'>
                        <div className='flex justify-between items-center'>
                          <div className='flex items-center gap-2 flex-1'><Shield color1={h?.color1} color2={h?.color2} initial={h?.name} size='xs' isFlag={h?.isFlag} /><span className={'text-[10px] font-bold uppercase italic truncate ' + 
(h ? 'text-white' : 'text-slate-400')}>{h?.name || 'TBD'}</span></div>
                        ...
                        <div className='flex justify-between items-center'>
                          <div className='flex items-center 
gap-2 flex-1'><Shield color1={a?.color1} color2={a?.color2} initial={a?.name} size='xs' isFlag={a?.isFlag} /><span className={'text-[10px] font-bold uppercase italic truncate ' + (a ? 'text-white' : 'text-slate-400')}>{a?.name ||
'TBD'}</span></div>
                          <div className='flex items-center gap-2 tabular-nums font-black italic text-xs bg-black/30 px-2 py-0.5 rounded'>
                        ...
                        {winner && (
                          <div className='mt-2 pt-2 border-t border-white/10 flex items-center justify-center gap-2 bg-emerald-900/30 
...
italic text-white truncate max-w-[100px]'>{winner.name}</span>
                          </div>
                        )}
                      </div>
                    );
...
rounded-[2.5rem] p-6 mb-4 border-b-4 border-slate-800 relative shadow-xl'>
...
font-bold text-slate-300 mt-1 bg-black/40 backdrop-blur-sm inline-block px-2 rounded'>{matchState.home.att + '/' + matchState.home.opp + '/' + matchState.home.def}</p>
            </div>
            ...
            <div className='px-4 flex flex-col items-center shrink-0 w-32'>
              ...
              {!matchState.finished && matchState.phase !== 'penalties' 
&& <div className='text-[8px] font-black text-white/70 uppercase italic mt-1 bg-black/40 px-2 py-0.5 rounded backdrop-blur-sm'>{matchState.oppH} vs {matchState.oppA} TIROS RESTANTES</div>}
...
text-blue-300 tabular-nums drop-shadow-md'>(pen {matchState.penalties.scoreH} - {matchState.penalties.scoreA})</div>
                  {matchState.penalties.shotsH < 5 ||
matchState.penalties.shotsA < 5 ? (
                  ...
                  <div className='text-[7px] font-bold text-slate-200 uppercase mt-1 bg-black/50 backdrop-blur-sm px-2 py-0.5 rounded text-center'>{matchState.penalties.phase === 'att' ?
'⚽ Preparando Disparo' : '🧤 ¡El portero se prepara!'}</div>
                </div>
              )}
            </div>
...
(
            <div className='z-10 flex flex-col items-center gap-8'>
              ...
              <button onClick={handleRoll} disabled={rolling} className='bg-white/90 backdrop-blur-sm text-emerald-900 px-10 py-5 
rounded-3xl font-black uppercase italic tracking-widest shadow-[0_10px_20px_rgba(0,0,0,0.5)] active:scale-90 transition-transform disabled:opacity-50'>{rolling ? 'Lanzando...' : 'Lanzar Dado'}</button>
            </div>
          ) : (
            <div className='z-10 text-center p-6 bg-black/40 backdrop-blur-md rounded-3xl border border-white/20 max-w-[80%] shadow-2xl'>
              ...
              <h3 className='text-lg font-black uppercase italic mb-4 text-white 
drop-shadow-md'>¡Fin del Partido!</h3>
     ...
     <div key={i} className={'text-[10px] font-bold italic flex gap-3 ' + (i === 0 ?
'text-white drop-shadow-md' : 'text-slate-300')}><span className='opacity-60 shrink-0'>⚽</span><p>{log}</p></div>
          ))}
        </div>
      </div>
    );
...
{Array.isArray(activeComp.teams) && activeComp.teams.map(t => (
            <button key={t.id} onClick={() => { updateActiveComp({ userTeamId: t.id }); setCompView('main'); }} className={'flex items-center gap-4 p-4 rounded-3xl border transition-all active:scale-95 backdrop-blur-md ' + (t.id === activeComp.userTeamId ? 'bg-blue-600/60 border-blue-400 shadow-[0_0_20px_rgba(37,99,235,0.5)]' : 'bg-slate-900/40 border-white/10 hover:border-white/30')}>
              ...
              <div className='text-left'>
                <p 
className='text-xs font-black uppercase italic text-white drop-shadow-md'>{t?.name}</p>
    ...
    <div className='relative min-h-screen selection:bg-blue-500/30 font-sans text-slate-100 overflow-hidden'>
      ...
      <div className='relative z-10 max-w-md mx-auto min-h-screen flex flex-col'>
        <AnimatePresence mode='wait'>
          {view === 'hub' && <motion.div key='hub' className='flex-grow flex flex-col' initial={{ opacity: 0 }} animate={{ opacity: 1 }} 
exit={{ opacity: 0 }}><HubView setView={setView} setActiveCompId={setActiveCompId} setCompView={setCompView} comps={comps} /></motion.div>}
          ...
          {view === 'competition' 
&& <motion.div key='comp' className='flex-grow flex flex-col' initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 1.1, opacity: 0 }}><CompetitionView /></motion.div>}
        </AnimatePresence>
      </div>
    </div>
  );
